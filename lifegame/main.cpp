...
int main() {
    ...
    game.state = Game::Accueil; // Le jeu commence sur l'ecran d'accueil
    ...
    // Charger l'image de fond pour l'ecran d'accueil
    ...
    // Case a cocher pour le mode torique
    ...
    // Variables pour la selection multiple
    ...
    // Variable pour le mode edition via le crayon
    ...
    // Barre laterale pour le score
    sf::RectangleShape sidebar(sf::Vector2f(sidebarWidth, height * cellSize));
    sidebar.setFillColor(sf::Color(50, 50, 50)); // Gris fonce
    ...
    while (window.isOpen() && isRunning) {
        if (game.state == Game::Accueil) {
            ...
            // Dessiner les elements de l'ecran d'accueil
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed) {
                // Verifier les combinaisons de touches directement
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                ...
                if (event.mouseButton.button == sf::Mouse::Left) {
                    // Si l'aide est visible
                    if (isHelpVisible) {
                        // Verifier si le clic est en dehors de la fenêtre d'aide
                        if (!(mouseX >= helpBackground.getPosition().x && mouseX <= helpBackground.getPosition().x + helpBackground.getSize().x &&
                              mouseY >= helpBackground.getPosition().y && mouseY <= helpBackground.getPosition().y + helpBackground.getSize().y)) {
                            isHelpVisible = false;
                        }
                    } else {
                        isMousePressed = true;
                        ...
                        // Gestion de la case a cocher pour le mode torique
                            ...
                            if (isToroidal) {
                                toroidalCheckbox.setFillColor(sf::Color::Green);
                            } else {
                                toroidalCheckbox.setFillColor(sf::Color::White);
                            }
                        }
                        ...
                        // Bouton "Undo"
                            ...
                            if (game.state == Game::Simulation) {
                                ...
                                // Annuler l'etape precedente
                                game.undo();
                            } else if (game.state == Game::Edition) {
                                // Rien (ou implementer l'annulation des modifications en mode edition)
                            }
                        }
        ...
        // Mise a jour de la grille en mode simulation
        if (timeSinceLastUpdate > timePerFrame) {
            if (game.state == Game::Simulation) {
                game.saveState();  // Sauvegarder l'etat actuel pour l'undo

                // Obtenir la grille precedente pour comparer
                Grid previousGrid = game.getGrid();

                game.getGrid().update();
                game.setIterations(game.getIterations() + 1);

                // Comparer la grille precedente et actuelle pour compter les cellules creees et detruites
        ...
        for (int x = 0; x < width; ++x) {
            for (int y = 0; y < height; ++y) {
                ...
                if (cell->isAlive()) {
                    ...
                    cellShape.setFillColor(sf::Color(64, 64, 64));  // Obstacles morts
                    cellShape.setPosition(x * cellSize, y * cellSize);
                    window.draw(cellShape);
                }
                // Les cellules mortes normales ne sont pas dessinees
            }
        }
        ...
        // Dessiner la case a cocher pour le mode torique
        ...
        if (isHelpVisible) {
            ...
            sf::Text playText("Bouton Play : Demarrer ou reprendre la simulation.", font, 14);
            ...
            sf::Text undoText("Bouton Undo : Mettre en pause ou annuler l'action precedente.", font, 14);
            ...
            sf::Text redoText("Bouton Redo : Refaire l'action annulee.", font, 14);
            ...
            sf::Text restartText("Bouton Restart : Reinitialiser la simulation.", font, 14);
            ...
            // Creer une nouvelle instance de l'icône du crayon pour la fenêtre d'aide
            ...
            sf::Text editText("Bouton Edit :\n- Jaune : Mode edition desactive.\n- Vert : Mode edition active.", font, 14);
            ...
            sf::Text toroidalText("Case a cocher : Activer/desactiver le mode torique.", font, 14);
            ...
            y += 20;
             ...
             // --- Clic droit ---
            sf::Text leftClickText("Clic gauch : Placer ou supprimer des cellules obstacles.", font, 14);
            ...
            sf::Text shortcut1("- Ctrl + P : Placer un planeur a la position de la souris.", font, 14);
            shortcut1.setFillColor(sf::Color::Black);
            shortcut1.setPosition(iconX + 20, y);

            y += 20;

            sf::Text shortcut2("- Ctrl + L + H : Placer une ligne horizontale.", font, 14);
            shortcut2.setFillColor(sf::Color::Black);
            shortcut2.setPosition(iconX + 20, y);

            y += 20;

            sf::Text shortcut3("- Ctrl + L + V : Placer une ligne verticale.", font, 14);
